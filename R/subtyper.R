...
compare_model_predictions <- function(model1, model2, data, response, re.form = NULL, custom_title=NULL ) {
  ...
  plot1 <- ggplot(plot_data, aes(x = True, y = Pred1)) +
    ...
    annotate(geom="label", x = Inf, y = Inf, label = paste("R² =", r_squared1),
             hjust = 1.1, vjust = 1.5, size = 5, color = "blue", fill = 'white') +
    coord_equal(ratio = 1) +
  ...
  plot2 <- ggplot(plot_data, aes(x = True, y = Pred2)) +
    ...
    annotate(geom="label", x = Inf, y = Inf, label = paste("R² =", r_squared2),
             hjust = 1.1, vjust = 1.5, size = 5, color = "blue", fill = 'white') +
