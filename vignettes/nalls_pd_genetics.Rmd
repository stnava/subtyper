...
nv = 8
nmfmat[ nmfmat == 0 ] = 0.00001 # this hack improves convergence
res <- nmf( nmfmat, nv )
w <- basis(res)
h <- coef(res)
basismap(res)
...
for ( j in 1:nv ) {
  ...
  print( pheatmap::pheatmap( abs(subviz), cluster_cols=F, cluster_rows=T, show_rownames=T, show_colnames=T ) )
}
```
