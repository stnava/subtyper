...
```{r functionalenrichment,echo=FALSE,eval=TRUE}
library( gprofiler2 )
library( subtyper )
...
## bi-clustering via NMF


```{r simple,echo=FALSE,eval=FALSE}
...
binmat = myviz2 * 0
binmat[ jointClusterMatrix == "1-2" ] = 1
pheatmap::pheatmap( myviz2 * binmat, cluster_cols=F, cluster_rows=F, show_rownames=F, show_colnames=F)
```
...
eanat = sparseDecom( eanatmat, sparseness = 0.1, nvecs = nv,
  its = 25, z = 0.1, mycoption = 0 )
qq = 0.5
for ( j in 1:nv ) {
  uvec = abs( eanat$umatrix[,j] )
  vvec = abs( eanat$eigenanatomyimages[j,] )
  uvec = uvec / sum( uvec )
  vvec = vvec / sum( vvec )
