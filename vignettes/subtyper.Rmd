...
```{r trainclustermodels}
cohortbl = mydf[ mydf$visit == 'V0', ]
mvcl='MVST' # default prefix - can be changed
cclusterobj = consensusSubtypingTrain( cohortbl, fnames, clustlist, 2,
  mvcl=mvcl, verbose=TRUE )
clustmodels = cclusterobj[["models"]]
reoModels = cclusterobj[["reorderers"]]
```


Predict the subtypes (clusters) on all time points.
```{r predictclustermodels}
mydf = consensusSubtypingPredict( mydf, fnames, clustlist, 
  clustmodels, reorderers = reoModels, 
  mvcl=mvcl, idvar='Id', visitName='visit', baselineVisit='V0')
cocanames = getNamesFromDataframe( mvcl, mydf, exclusions="_mem")
cocanames=cocanames[ cocanames != mvcl ]
```
...
clustdata = consensusSubtypingCOCA( mydf, 2, cocanames,
    newclustername=ccname, 
    idvar='Id', visitName='visit', baselineVisit='V0', verbose=TRUE )
```
...
table( clustdata[,cocanames[k]], clustdata[,cocanames[3]])
table( clustdata[,cocanames[k]], clustdata[,ccname])
```
